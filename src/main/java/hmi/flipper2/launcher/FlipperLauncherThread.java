...
public class FlipperLauncherThread extends Thread {
    ...
    private void init(Properties ps) {
        ...
        this.showISViewer = Boolean.parseBoolean(ps.getProperty("isViewer"));
        // Database
    ...
    /**
     * Method for sending updates on the IS to the GUI.
     */
    public void updateGUI(){
        try {        
            this.viewer.updateTree(tc.getIs("is"));
        } catch (FlipperException ex) {
            java.util.logging.Logger.getLogger(FlipperLauncherThread.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    /**
     * Logging the IS in the console. By default only when Flipper is run in debug mode
     * @param stepMarker, the number of template checks
     */
